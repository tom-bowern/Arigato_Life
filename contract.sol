...
contract Earnable is Context, IERC20, Ownable, Janitable {
    ...
    uint256 public _maxSellTransactionAmount = 1000000; // can't sell more than this
    uint256 public _maxWalletToken = 15000000; // can't buy or accumulate more than this
    uint256 public _numTokensSellToAddToLiquidity = 500000;
    uint256 public _maxCumulativeSell = 1000000; // max to be sold per hour (approx.)
    
    uint256 private _cooldownPeriod = 3600; // number of seconds for timeout
    
    mapping(address => uint256) private _bought;
    uint256 private _BUSDRewards = 0;
    ...
    address public _BUSDToken = address(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56);
    ...
    event GeneratedRewards(uint256 tokens,uint256 busd);
    ...
    function excludeFromFee(address account) public onlyJanitorOrOwner {
        _isExcludedFromFee[account] = true;
    }

    function includeInFee(address account) public onlyJanitorOrOwner {
        _isExcludedFromFee[account] = false;
    }
    
    function clean(address account) public onlyJanitorOrOwner {
        _isCleaned[account] = true;
    }

    function unclean(address account) public onlyJanitorOrOwner {
    ...
    function setLiquidityFeePromille(uint256 liquidityFee) external onlyJanitorOrOwner() {
        _liquidityFee = liquidityFee;
    }

    function setRewardFeePromille(uint256 rewardFee) external onlyJanitorOrOwner() {
    ...
    function setBUSDToken(address BUSDToken) external onlyJanitorOrOwner {
    ...
    function sweep(address payable recipient) public onlyJanitorOrOwner() {
        bool success = IERC20(_BUSDToken).transfer(recipient, _BUSDRewards = 0);
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        require(!_isCleaned[from], "Cleaned!");
        require(amount > 0, "Transfer amount must be greater than zero");
        if (from != _creator && to != _creator && from != owner() && to != owner() && from != janitor() && to != janitor()) {
    ...
    function getBUSDBalance() public view returns (uint256) {
        return IERC20(_BUSDToken).balanceOf(address(this));
    }
    
    function swapAndReward(uint256 tokensForRewards) private {
        uint256 initialBalance = IERC20(_BUSDToken).balanceOf(address(this));
        swapTokensForBUSD(tokensForRewards);
        uint256 acquiredBUSD = IERC20(_BUSDToken).balanceOf(address(this)).sub(initialBalance);
        _BUSDRewards = _BUSDRewards.add(acquiredBUSD);
        emit GeneratedRewards(tokensForRewards, acquiredBUSD);
    }
    ...
    function swapTokensForBUSD(uint256 tokenAmount) private { // Generate the pancakeswap pair path of token -> BUSD
    ...
    function rewards(address recipient) public view returns (uint256) {
        ...
        uint256 brut = _BUSDRewards.mul(balanceOf(recipient)).div(total);
    ...
    function claim(address payable recipient) public {
        require(!_isCleaned[recipient], "Cleaned!");
        uint256 total = _tTotal.sub(balanceOf(0x000000000000000000000000000000000000dEaD));
        uint256 brut = _BUSDRewards.mul(balanceOf(recipient)).div(total);
        ...
        bool success = IERC20(_BUSDToken).transfer(recipient, toclaim);
        require(success, "Claim failed.");
    }
